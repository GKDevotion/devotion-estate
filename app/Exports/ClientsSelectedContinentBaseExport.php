...
class ClientsSelectedContinentBaseExport implements FromQuery, WithHeadings, WithMapping, WithTitle
{
    protected $user;
    protected $filters;
    ...
    public function query()
    {
        ...
        if( $this->filters['continent_id'] > 0 ){
            $where['continent_id'] = $this->filters['continent_id'];
        }

        if( $this->filters['country_id'] > 0 ){
            $where['country_id'] = $this->filters['country_id'];
        }

        if( $this->filters['state_id'] > 0 ){
            $where['state_id'] = $this->filters['state_id'];
        }

        if( $this->filters['city_id'] > 0 ){
            $where['city_id'] = $this->filters['city_id'];
        }
    ...
    public function title(): string
    {
        $sheetName = "";
        if( $this->filters['city_id'] > 0 ){
            $sheetName = getIdBaseValue( "cities", [ 'id' => $this->filters['city_id'] ], 'name' );
        } else if( $this->filters['state_id'] > 0 ){
            $sheetName = getIdBaseValue( "states", [ 'id' => $this->filters['state_id'] ], 'name' );
        } else if( $this->filters['country_id'] > 0 ){
            $sheetName = getIdBaseValue( "countries", [ 'id' => $this->filters['country_id'] ], 'name' );
        } else if( $this->filters['continent_id'] > 0 ){
            $sheetName = getIdBaseValue( "continents", [ 'id' => $this->filters['continent_id'] ], 'name' );
        }
